...
aggregate_crsp_variable <- function(tna_data,other_variable_data,variable,id_var){
  ...
  temp_data <- temp_data[order(temp_data[,id_var], 
                               ...
                               temp_data[,"yr"], 
                               temp_data[,"month"]),] 
  row.names(temp_data) <- seq(nrow(temp_data))
  
  #temp_data_trim <- subset(temp_data,select=-c(crsp_fundno))
  temp_data_trim <- temp_data_trim[,!(colnames(temp_data_trim) %in% "crsp_fundno")]
  ...
  row.names(temp_data_trim) <- seq(nrow(temp_data_trim))
  ...
  row.names(temp_data_sum_tna) <- seq(nrow(temp_data_sum_tna))
  ...
  temp_data_weights <- merge(temp_data[,!(colnames(temp_data) %in% "mtna")], temp_data_weights0, 
                             by.x=c("crsp_fundno","yr","month"), by.y=c("crsp_fundno","yr","month"), 
                             all.x=TRUE, all.y=FALSE, sort=FALSE, suffixes=c(".x",".y"),incomparables=NA)
  ...
  row.names(temp_data_weights) <- seq(nrow(temp_data_weights))
  ...
  row.names(temp_data_agg1_trim) <- seq(nrow(temp_data_agg1_trim))
...
calculate_similarity_by_group <- function(merged_data,group_var,group_var_value,text_type,id_var){
  ...
  for (k in 1:length(text_percentages))
  {
    ...
    for (l in 1:length(temp_years))
    {
      ...
      row.names(temp_data_obj) <- seq(nrow(temp_data_obj))
      ...
      row.names(temp_data_full) <- seq(nrow(temp_data_full))
    ...
    temp_stacked <- merge(temp_stacked, temp_avg[,!(colnames(temp_avg) %in% "group")], 
                          by.x=c(id_var,"yr"), by.y=c(id_var,"yr"), 
                          all.x=TRUE, all.y=FALSE, sort=FALSE, suffixes=c(".x",".y"),incomparables=NA)
